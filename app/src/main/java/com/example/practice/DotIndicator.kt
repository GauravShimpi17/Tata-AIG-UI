...
class DotIndicator(
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        val activePosition = if (parent.layoutManager is LinearLayoutManager) {
    ...
    private fun drawInactiveDots(
        c: Canvas, indicatorStartX: Float, indicatorPosY: Float, itemCount: Int
    ) {
    ...
    private fun drawActiveDot(
        c: Canvas, indicatorStartX: Float, indicatorPosY: Float, highlightPosition: Int
    ) {
        // width of item indicator including padding
        val itemWidth = (this.radius * 2 + indicatorItemPadding).toFloat()
